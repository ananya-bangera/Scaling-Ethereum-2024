...
app.get("/scores/:match_id/:players", (request, response) => {
  let scores = new Map();
  MATCH_DATA[parseInt(request.params.match_id) - 1]["batsmen"].forEach(
    (player) => {
      var score =
        player["runs"] * 2 +
        player["4s"] * 4 +
        player["6s"] * 6 +
        (player["out/not_out"] == "out" ? -1 : 1) * 15;
      scores.set(player["player_id"], score);
    }
  );
  MATCH_DATA[parseInt(request.params.match_id) - 1]["bowlers"].forEach(
    (player) => {
      var score =
        player["maiden"] * 8 +
        player["wickets"] * 6 +
        parseInt(player["economy"]) * -2 +
        player["0s"] * 2 +
        player["4s"] * -2 +
        player["6s"] * -4 +
        player["wides"] * -1 +
